...
const schema = yup
  .object({
    ...
    email: yup
      .string()
      .email()
      .test('dot', 'Email must contain a dot (".")', (value) => {
  ...
  .shape({
    picture: yup
      .mixed()
      .test('isExist', 'Please upload a file.', (value) => value[0]?.name)
      .test('fileSize', 'File too large. Maximum file size is 5 MB.', (value) => value[0] && value[0].size <= FILE_SIZE)
      .test('fileFormat', 'Unsupported image format', (value) => value[0] && SUPPORTED_FORMATS.includes(value[0].type))
  })
  .required();
    ...
    <section className='post-request-section'>
      ...
      {!isRegistered && !isLoading && (
        <form onSubmit={handleSubmit(onSubmit)} className='post-request-section__form'>
          ...
          {errors.position && (
            <span className='post-request-section__helped-text post-request-section--error-text'>
